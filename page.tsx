...
// Simple RFC-5322-ish email check — good enough to catch typos and garbage input
// without being overly strict about edge-case valid addresses.
            ...
            <div className="flex flex-col gap-10 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-xl">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <motion.a
                    href="mailto:kingdonhouse009@gmail.com"
                    whileHover={{ scale: 1.04, y: -2 }}
              ...
              <form
                ...
                noValidate
                className="w-full max-w-md rounded-[1.5rem] border border-white/10 bg-slate-950/60 p-6 backdrop-blur-2xl sm:p-8"
              >
                <div className="flex flex-col gap-5">
                  <div>
                    <label htmlFor="name" className="mb-2 block text-xs uppercase tracking-[0.3em] text-slate-400">
                      Name
                    </label>
                    <input
                      id="name"
                      ...
                      className={`w-full rounded-xl border bg-white/5 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400/60 focus:ring-2 focus:ring-cyan-400/20 ${
                        formErrors.name ? "border-red-400/60" : "border-white/10"
                      }`}
                      placeholder="Your name"
                    />
                    {formErrors.name && <p className="mt-1.5 text-xs text-red-300">{formErrors.name}</p>}
                  </div>

                  <div>
                    <label htmlFor="email" className="mb-2 block text-xs uppercase tracking-[0.3em] text-slate-400">
                      Email
                    </label>
                    <input
                      id="email"
                      ...
                      className={`w-full rounded-xl border bg-white/5 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400/60 focus:ring-2 focus:ring-cyan-400/20 ${
                        formErrors.email ? "border-red-400/60" : "border-white/10"
                      }`}
                      placeholder="you@example.com"
                    />
                    {formErrors.email && <p className="mt-1.5 text-xs text-red-300">{formErrors.email}</p>}
                  </div>

                  <div>
                    <label htmlFor="message" className="mb-2 block text-xs uppercase tracking-[0.3em] text-slate-400">
                      Message
                    </label>
                    <textarea
                      id="message"
                      ...
                      className={`w-full resize-none rounded-xl border bg-white/5 px-4 py-3 text-sm text-slate-100 outline-none transition focus:border-cyan-400/60 focus:ring-2 focus:ring-cyan-400/20 ${
                        formErrors.message ? "border-red-400/60" : "border-white/10"
                      }`}
                      placeholder="Tell me about your project..."
                    />
                    {formErrors.message && <p className="mt-1.5 text-xs text-red-300">{formErrors.message}</p>}
                  </div>

                  <motion.button
                    type="submit"
                    disabled={submitStatus === "sending"}
                    whileHover={submitStatus !== "sending" ? { scale: 1.02, y: -2 } : {}}
                    whileTap={submitStatus !== "sending" ? { scale: 0.98 } : {}}
                    className="inline-flex items-center justify-center gap-2 rounded-full border border-cyan-400/50 bg-gradient-to-r from-cyan-400/25 to-sky-400/20 px-6 py-3 text-sm font-semibold text-white shadow-[0_0_30px_rgba(0,217,255,0.2)] backdrop-blur-xl disabled:cursor-not-allowed disabled:opacity-60"
                  >
